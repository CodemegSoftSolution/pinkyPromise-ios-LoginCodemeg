...
class TYHeightPicker: UIView {
    ...
    public var separatorLineView, indicatorView, pointerView: UIView!
    public var cmBtn, feetInchBtn: UIButton!
    ...
    private var selectedCM: CGFloat = 0 {
        didSet {
            self.resultLabel.text = "\(Int(selectedCM)) Kg"
        }
    }
    ...
    override init(frame: CGRect) {
        ...
        pointerView = createView(ColorPink)
    ...
    func setDefaultHeight(_ height: CGFloat, unit: HeightUnit) {
        ...
        animateIndicatorView(selectedIndex)
        print("index = ", selectedCM)
        if selectedIndex == 0 {
            selectedCM = height
            delegate?.selectedHeight(height: selectedCM, unit: .CM)
            let offset = CGPoint(x: (selectedCM-17) * cellWidthIncludingSpacing - collectionView.contentInset.left, y: -collectionView.contentInset.top)
            print(offset)
            collectionView.setContentOffset(offset, animated: true)
            ...
            selectedInch = height
            delegate?.selectedHeight(height: selectedInch, unit: .Inch)
            let offset = CGPoint(x: (selectedInch-17) * cellWidthIncludingSpacing - collectionView.contentInset.left, y: -collectionView.contentInset.top)
            print(offset)
            collectionView.setContentOffset(offset, animated: true)
        }
    }
    ...
    override func layoutSubviews() {
        ...
        pointerView.heightAnchor.constraint(equalToConstant: 50).isActive = true
...
class TYLineCell: UICollectionViewCell {
    ...
    let lineView: UIView = {
        let view = UIView()
        view.backgroundColor = UIColor.init(red: 1/255.0, green: 4/255.0, blue: 13/255.0, alpha: 0.2)//ColorPink rgba(1, 4, 13, 1)
        return view
    }()
    ...
    var numberLabel: UILabel = {
        ...
        lbl.textColor = UIColor.init(red: 117/255.0, green: 129/255.0, blue: 143/255.0, alpha: 1)
        lbl.textAlignment = .center
        return lbl
    }()
