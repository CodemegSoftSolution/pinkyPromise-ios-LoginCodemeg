...
public class APIManager {
    ...
    func getJsonHeaderWithToken() -> [String:String]{
        return ["Content-Type":"application/json", "access_token":AppModel.shared.token]
    }
    
    func getMultipartHeaderWithToken() -> [String:String]{
        return ["Content-Type":"multipart/form-data", "access_token":AppModel.shared.token]
    }
        ...
        AF.upload(multipartFormData: { (multipartFormData) in
            ...
            case .success(let dict):
                ...
                if let result = dict as? [String: Any] {
                    if let code = result["status"] as? Int{
                        if(code == 200){
            ...
            AF.request(api, method: .post, parameters: params, encoding: JSONEncoding.default, headers: headerParams).responseJSON { (response) in
                ...
                case .success(let dict):
                    ...
                    if let result = dict as? [String: Any] {
                        ...
                        if let code = result["satus"] as? Int {
                            if(code == 200) {
                                DispatchQueue.main.async {
                                    completion(response.data)
                                }
    ...
    //MARK:- I AM COOL
    func post_Service_Call(params: [String: Any],api: String,Loader: Bool,isMultipart:Bool,_ completion: @escaping (_ dictArr: Data?) -> Void){
        if !APIManager.isConnectedToNetwork()
        {
            APIManager().networkErrorMsg()
            return
        ...
        DispatchQueue.global().async {
            ...
            AF.request(api, method: .post, parameters: params, encoding: JSONEncoding.default, headers: headerParams).responseJSON { (response) in
                ...
                case .success(let dict):
                    ...
                    log.ln("prettyJSON End \n")/
                    if let result = dict as? [String: Any] {
                        if let status = result["result"] as? Bool {
                            if status {
                                DispatchQueue.main.async {
                                    completion(response.data)
                                }
